...
namespace PluginMaster
{
    ...
    public class PointOctreeNode<T>
    {
        ...
        public int BestFitChild( Vector3 objPos ) => ( objPos.x <= Center.x ? 0 : 1 ) + ( objPos.y >= Center.y ? 0 : 4 ) + ( objPos.z <= Center.z ? 0 : 2 );
        ...
        public void GetNearby( ref Ray ray, float maxDistance, List<T> result )
        {
            ...
            for ( int i = 0; i < objects.Count; i++ )
            {
                if ( objects[ i ].Obj                             != null
                     && SqrDistanceToRay( ray, objects[ i ].Pos ) <= maxDistance * maxDistance )
                {
