...
namespace PluginMaster
{
    ...
    public class TilingSettings : PaintOnSurfaceToolSettings, IPaintToolSettings
    {
        ...
        {
            TilingSettings clone = new TilingSettings();
            clone.Copy( this );
            return clone;
        }
        ...
        {
            TilingSettings otherTilingSettings = other as TilingSettings;
            base.Copy( other );
        ...
        public Quaternion rotation
        {
            ...
            set
            {
                ...
                Quaternion prevRotation = _rotation;
                _rotation = value;
                OnDataChanged();
                if ( OnRotationChanged != null )
                {
                    float   angle = Quaternion.Angle( prevRotation, _rotation );
                    Vector3 axis  = Vector3.Cross( prevRotation * Vector3.forward, _rotation * Vector3.forward );
                    if ( axis == Vector3.zero )
                    {
                        axis = Vector3.Cross( prevRotation * Vector3.up, _rotation * Vector3.up );
                    }
        ...
        public AxesUtils.SignedAxis axisAlignedWithNormal
        {
            ...
            set
            {
                if ( _axisAlignedWithNormal == value )
                {
                    return;
                }
        ...
        public void UpdateCellSize()
        {
            ...
            if ( _cellSizeType != CellSizeType.CUSTOM )
            {
                ...
                Vector3 cellSize = Vector3.one
                                   * ( _cellSizeType == CellSizeType.SMALLEST_OBJECT
                                       ? float.MaxValue
                                       : float.MinValue );

                foreach ( MultibrushItemSettings item in selectedBrush.items )
                {
                    ...
                    Vector3 scaleMultiplier = _cellSizeType == CellSizeType.SMALLEST_OBJECT
                        ? item.minScaleMultiplier
                        : item.maxScaleMultiplier;
                    if ( overwriteBrushProperties )
                    {
                        scaleMultiplier = Vector3.Scale( scaleMultiplier, brushSettings.scaleMultiplier );
                    }

                    Vector3 itemSize = Vector3.Scale( BoundsUtils.GetBoundsRecursive( prefab.transform,
                    ...
                    cellSize = _cellSizeType == CellSizeType.SMALLEST_OBJECT
                        ? Vector3.Min( cellSize, itemSize )
                        : Vector3.Max( cellSize, itemSize );
                }

                if ( _axisAlignedWithNormal.axis == AxesUtils.Axis.Y )
                {
    ...
    public class TilingToolName : IToolName
    {

        #region Public Properties

        public string value => "Tiling";
    ...
    public class TilingData : PersistentData<TilingToolName, TilingSettings, ControlPoint>
    {
        ...
        private static TilingData _instance;
        ...
        {
            TilingData clone = new TilingData();
            base.Clone( clone );
        ...
        [NonSerialized]
        private List<Vector3> _tilingCenters
            = new List<Vector3>();
    ...
    public static partial class PWBIO
    {
        ...
        private static TilingData _initialPersistentTilingData;
        private static TilingData _selectedPersistentTilingData;
        ...
        private static void ApplySelectedPersistentTiling( bool deselectPoint )
        {
            if ( !ApplySelectedPersistentObject( deselectPoint, ref _editingPersistentTiling, ref _initialPersistentTilingData,
                    ref _selectedPersistentTilingData,          TilingManager.instance ) )
            {
                return;
            }

            if ( _initialPersistentTilingData == null )
            {
                return;
            }

            TilingData selectedTiling = TilingManager.instance.GetItem( _initialPersistentTilingData.id );
            _initialPersistentTilingData = selectedTiling.Clone();
        }

        private static void ApplyTilingHandlePosition( TilingData data ) => SetTilingSelectedPoint( data, handlePosition );
        ...
        {
            string                               nextTilingId = TilingData.nextHexId;
            Dictionary<string, List<GameObject>> objDic       = Paint( TilingManager.settings, PAINT_CMD, true, false, nextTilingId );
            if ( objDic.Count != 1 )
            {
                return;
            }
            ...
            GameObject[] objs           = objDic[ nextTilingId ].ToArray();
            TilingData   persistentData = new TilingData( objs, initialBrushId, _tilingData );
                                                     ...
                                                     out bool   clickOnPoint, out bool wasEdited, out Vector3 delta )
        {
            ...
            if ( data.selectedPointIdx < 0 )
            {
                return false;
            }

            Vector3 prevPoint = data.selectedPoint;
            wasEdited = SetTilingSelectedPoint( data,
                Handles.PositionHandle( data.selectedPoint, data.settings.rotation ) );
            if ( prevPoint != data.selectedPoint )
            {
                ToolProperties.RepainWindow();
            }
            ...
            {
                Quaternion prevRotation = data.settings.rotation;
                wasEdited = SetTilingRotation( data,
                    Handles.RotationHandle( data.settings.rotation, data.GetPoint( 8 ) ) );
                if ( prevRotation != data.settings.rotation )
                {
                    ToolProperties.RepainWindow();
                }
            }
        ...
        {
            TilingSettings settings = data.settings;
            Vector3[] cornerPoints = new[]
            {
                data.GetPoint( 0 ), data.GetPoint( 1 ),
        ...
        private static void PreviewPersistentTiling( TilingData data )
        {
            ...
            Vector3[]        objPos   = null;
            List<GameObject> objList  = data.objectList;
            TilingSettings   settings = data.settings;
            ...
            for ( int objIdx = 0; objIdx < objPos.Length; ++objIdx )
            {
                ...
                Ray     ray          = new Ray( itemPosition + Vector3.up * height, Vector3.down );
                ...
                Quaternion    itemRotation  = settings.rotation;
                ...
                Vector3 axisAlignedWithNormal = (Vector3)settings.axisAlignedWithNormal;
                if ( settings.axisAlignedWithNormal.axis != AxesUtils.Axis.Y )
                {
                    axisAlignedWithNormal *= -1;
                }

                itemRotation *= Quaternion.FromToRotation( Vector3.up, axisAlignedWithNormal );
                Quaternion previewRotation = itemRotation;
                ...
                Vector3 pivotToCenter = itemRotation * obj.transform.InverseTransformPoint( bounds.center );
        ...
        private static void RotateTiling( TilingData data, float angle, Vector3 axis, bool updateDataRotation )
        {
            ...
            for ( int i = 0; i < 8; ++i )
            {
                Vector3 centerToPoint = data.GetPoint( i )    - data.GetPoint( 8 );
                Vector3 rotatedPos    = delta * centerToPoint + data.GetPoint( 8 );
                data.SetPoint( i, rotatedPos, registerUndo: false, selectAll: false );
            }

            if ( updateDataRotation )
            {
                data.settings.rotation *= delta;
            }
        ...
        private static bool SetTilingRotation( TilingData data, Quaternion rotation )
        {
            ...
            float angle = Quaternion.Angle( prevRotation, data.settings.rotation );
            Vector3 axis = Vector3.Cross( prevRotation * Vector3.forward,
                data.settings.rotation                 * Vector3.forward );
            if ( axis == Vector3.zero )
            {
                axis = Vector3.Cross( prevRotation * Vector3.up,
        ...
        private static bool SetTilingSelectedPoint( TilingData data, Vector3 position )
        {
            ...
            Vector3 prevPosition = data.selectedPoint;
            Vector3 snappedPoint = SnapAndUpdateGridOrigin( handlePosition, SnapManager.settings.snappingEnabled,
            ...
            handlePosition = data.selectedPoint;
            ...
            updateStroke = true;
            Vector3 delta = data.selectedPoint - prevPosition;
            if ( data.selectedPointIdx < 4 )
            {
                int nextCornerIdx     = ( data.selectedPointIdx + 1 ) % 4;
                int oppositeCornerIdx = ( data.selectedPointIdx + 2 ) % 4;
                int prevCornerIdx     = ( data.selectedPointIdx + 3 ) % 4;

                Vector3 nextVector  = data.GetPoint( nextCornerIdx ) - prevPosition;
                Vector3 prevVector  = data.GetPoint( prevCornerIdx ) - prevPosition;
                Vector3 deltaNext   = Vector3.Project( delta, nextVector );
                Vector3 deltaPrev   = Vector3.Project( delta, prevVector );
                Vector3 deltaNormal = delta                 - deltaNext - deltaPrev;
            ...
            {
                int     prevCornerIdx      = data.selectedPointIdx - 4;
                int     nextCornerIdx      = ( data.selectedPointIdx - 3 ) % 4;
                int     oppositeSideIdx    = ( data.selectedPointIdx - 2 ) % 4 + 4;
                Vector3 parallel           = data.GetPoint( nextCornerIdx )    - data.GetPoint( prevCornerIdx );
                Vector3 perpendicular      = data.GetPoint( oppositeSideIdx )  - prevPosition;
                Vector3 deltaParallel      = Vector3.Project( delta, parallel );
                Vector3 deltaPerpendicular = Vector3.Project( delta, perpendicular );
                Vector3 deltaNormal        = delta - deltaParallel - deltaPerpendicular;
                for ( int i = 0; i < 4; ++i )
                {
                    data.AddValue( i, deltaParallel + deltaNormal );
                }
        ...
        private static void TilingDuringSceneGUI( SceneView sceneView )
        {
                 ...
                 && Event.current.keyCode == KeyCode.Escape )
            {
                if ( _tilingData.state               == ToolManager.ToolState.EDIT
                     && _tilingData.selectedPointIdx > 0 )
                {
                    _tilingData.selectedPointIdx = -1;
                }
                else if ( _tilingData.state == ToolManager.ToolState.NONE )
                {
            ...
            if ( ToolManager.editMode
                 || TilingManager.instance.showPreexistingElements )
            {
        ...
        private static bool TilingShortcuts( TilingData data )
        {
            ...
            bool spacing1 = PWBSettings.shortcuts.tilingEditSpacing1.Check();
            bool spacing2 = PWBSettings.shortcuts.tilingEditSpacing2.Check();
            if ( spacing1 || spacing2 )
            {
                float delta = spacing1
                    ? PWBSettings.shortcuts.tilingEditSpacing1.combination.delta
                    : -PWBSettings.shortcuts.tilingEditSpacing2.combination.delta;
                float            deltaSign = -Mathf.Sign( delta );
                AxesUtils.Axis[] otherAxes = AxesUtils.GetOtherAxes( AxesUtils.Axis.Y );
                ...
                int     axisIdx  = spacing1 ? 1 : 0;
                Vector3 size     = data.GetPoint( 2 ) - data.GetPoint( axisIdx );
                float   axisSize = AxesUtils.GetAxisValue( size, otherAxes[ axisIdx ] );
        ...
        private static void TilingStateEdit( Camera camera )
        {
            ...
            {
                TilingStrokePreview( camera, TilingData.nextHexId, forceStrokeUpdate );
            }
        ...
        private static void TilingStateRectangle( bool in2DMode )
        {
            ...
            Plane plane    = new Plane( settings.rotation * Vector3.up, _tilingData.GetPoint( 0 ) );

            if ( plane.Raycast( mouseRay, out float distance ) )
            {
                Vector3 point = mouseRay.GetPoint( distance );
                _tilingData.SetPoint( 2, point, registerUndo: false, selectAll: false );
                Vector3 diagonal  = point - _tilingData.GetPoint( 0 );
                Vector3 tangent   = Vector3.Project( diagonal, settings.rotation * Vector3.right );
                Vector3 bitangent = Vector3.Project( diagonal, settings.rotation * Vector3.forward );
        ...
        private static void TilingStrokePreview( Camera camera, string hexId, bool forceUpdate )
        {
            ...
            if ( PreviewIfBrushtrokestaysTheSame( out brushstroke, camera, forceUpdate ) )
            {
                return;
            }
            ...
            for ( int i = 0; i < brushstroke.Length; ++i )
            {
                ...
                Vector3    size               = additionalRotation * Vector3.Scale( bounds.size, brushSettings.scaleMultiplier );
                size = new Vector3( Mathf.Abs( size.x ), Mathf.Abs( size.y ), Mathf.Abs( size.z ) );
                Vector3 pivotToCenter = prefab.transform.InverseTransformDirection( bounds.center - prefab.transform.position );
                ...
                Matrix4x4 previewRootToWorld = Matrix4x4.TRS( itemPosition, previewRotation, strokeItem.scaleMultiplier )
        ...
        {
            TilingData[]     persistentItems       = TilingManager.instance.GetPersistentItems();
            List<TilingData> deselectedItems       = new List<TilingData>( persistentItems );
            bool             clickOnAnyPoint       = false;
            bool             selectedItemWasEdited = false;
            foreach ( TilingData itemData in persistentItems )
            {
                ...
                long selectedTilingId = _initialPersistentTilingData == null ? -1 : _initialPersistentTilingData.id;
            ...
            if ( clickOnAnyPoint )
            {
                foreach ( TilingData itemData in deselectedItems )
                {
            ...
            {
                GameObject parent = _selectedPersistentTilingData.GetParent();
                if ( parent != null )
                {
        ...
        private static void UpdateCellCenters( TilingData data, bool DrawCells )
        {
            ...
            TilingSettings settings    = data.settings;
            Vector3        tangentDir  = data.GetPoint( 1 ) - data.GetPoint( 0 );
            float          tangentSize = tangentDir.magnitude;
            tangentDir.Normalize();
            Vector3 bitangentDir  = data.GetPoint( 3 ) - data.GetPoint( 0 );
            float   bitangentSize = bitangentDir.magnitude;
            bitangentDir.Normalize();
            Vector3   cellTangent   = tangentDir   * Mathf.Abs( settings.cellSize.x );
            Vector3   cellBitangent = bitangentDir * Mathf.Abs( settings.cellSize.y );
            Vector3[] vertices      = new[] { Vector3.zero, cellTangent, cellTangent + cellBitangent, cellBitangent };
            Vector3   offset        = data.GetPoint( 0 );
            ...
            void SetTileCenter()
            {
                ...
                for ( int i = 0; i <= 4; ++i )
                {
                    linePoints[ i ] = vertices[ i % 4 ] + offset;
                }

                Vector3 cellCenter = linePoints[ 0 ] + ( linePoints[ 2 ] - linePoints[ 0 ] ) / 2;
            ...
            Vector2 minCellSize = settings.cellSize + settings.spacing;
            minCellSize = Vector2.Max( minCellSize, Vector2.one * 0.001f );
            Vector2 cellSize      = minCellSize - settings.spacing;
            ...
            while ( Mathf.Abs( tangentOffset ) + Mathf.Abs( cellSize.x ) < tangentSize )
            {
                ...
                while ( Mathf.Abs( bitangentOffset ) + Mathf.Abs( cellSize.y ) < bitangentSize )
                {
                    ...
                    offset = data.GetPoint( 0 )
                             + tangentDir   * Mathf.Abs( tangentOffset )
        ...
        private static void UpdateMidpoints( TilingData data )
        {
            ...
            {
                int     nextI     = ( i + 1 ) % 4;
                Vector3 point     = data.GetPoint( i );
                Vector3 nextPoint = data.GetPoint( nextI );
