    ...
    public class PWBPreferences : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            using ( var scrollView = new EditorGUILayout.ScrollViewScope( _mainScrollPosition,
                       false, false, GUI.skin.horizontalScrollbar, GUI.skin.verticalScrollbar, EditorStyles.helpBox ) )
            {
        ...
        {
            _dataGroupOpen
                = EditorGUILayout.BeginFoldoutHeaderGroup( _dataGroupOpen, "Data Settings" );
            if ( _dataGroupOpen ) DataGroup();
            EditorGUILayout.EndFoldoutHeaderGroup();

            _autoSaveGroupOpen
                = EditorGUILayout.BeginFoldoutHeaderGroup( _autoSaveGroupOpen, "Auto-Save Settings" );
            ...
            _tempCollidersGroupOpen = EditorGUILayout.BeginFoldoutHeaderGroup( _tempCollidersGroupOpen,
                "Temp Colliders" );
            ...
            _gravityToolGroupOpen = EditorGUILayout.BeginFoldoutHeaderGroup( _gravityToolGroupOpen, "Gravity Tool" );
            ...
            _palettesGroupOpen = EditorGUILayout.BeginFoldoutHeaderGroup( _palettesGroupOpen, "Palettes" );
        ...
        private void DataGroup()
        {
            using ( new GUILayout.HorizontalScope( EditorStyles.helpBox ) )
            {
                ...
                EditorGUILayout.LabelField( "Data directory:"
                    , PWBSettings.fullDataDir, EditorStyles.textField );
                if ( GUILayout.Button( "...", GUILayout.Width( 29 ), GUILayout.Height( 20 ) ) )
                {
                    var directory = EditorUtility.OpenFolderPanel( "Select data directory...",
                        PWBSettings.fullDataDir, "Data" );
                    if ( Directory.Exists( directory ) ) PWBSettings.SetDataDir( directory );
                }
            }
        }
        ...
        private void GravityToolGroup()
        {
            using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
            {
                ...
                GravityToolManager.surfaceDistanceSensitivity
                    = EditorGUILayout.Slider( "Distance to surface sensitivity:",
        ...
        private void PalettesGroup()
        {
            using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
            {
                PWBCore.staticData.selectTheNextPaletteInAlphabeticalOrder
                    = EditorGUILayout.ToggleLeft( "Select the next palette in alphabetical order",
                        PWBCore.staticData.selectTheNextPaletteInAlphabeticalOrder );
            }
        }

        private static readonly string[] _tempCollidersActionNames =
        {
            "Never create temp colliders",
        ...
        private void Shortcuts()
        {
            ...
            {
                if ( (object)shortcut == (object)_selectedShortcut ) return EditorStyles.textField;
                return EditorStyles.label;
            }

            var categoryButton = new GUIStyle( EditorStyles.toolbarButton );
            ...
            using ( new GUILayout.HorizontalScope() )
            {
                ...
                using ( new GUILayout.VerticalScope() )
                {
                    ...
                    void ContextMenu( PWBShortcut shortcut, GenericMenu.MenuFunction DisableFunction )
                    {
                    ...
                    void ShortcutRow( PWBKeyShortcut shortcut )
                    {
                        ...
                        EditorGUI.DrawRect( rowRect, row % 2 == 0 ? _darkerColor : _lighterColor );
                        ...
                        cellRect.x += minX;
                        EditorGUI.LabelField( cellRect, new GUIContent( shortcut.name ) );
                        ...
                        cellRect.x     += minX;
                        cellRect.width -= 20;
                        EditorGUI.LabelField( cellRect, new GUIContent( shortcutString( shortcut ) ),
                        ...
                        cellRect.width =  20;
                        EditorGUI.LabelField( cellRect, new GUIContent( warningTexture ) );
                    }
                    ...
                    void MouseShortcutRow( PWBMouseShortcut shortcut, bool scrollWheelOnly = false )
                    {
                        ...
                        int                             modId      = Array.IndexOf( _modifierOptions, shortcut.combination.modifiers );
                        ...
                        {
                            modId = EditorGUI.Popup( cellRect, modId,
                        ...
                        {
                            if ( scrollWheelOnly ) EditorGUI.LabelField( cellRect, "+ Mouse scroll wheel" );
                            else
                            {
                                using ( var check = new EditorGUI.ChangeCheckScope() )
                                {
                                    mouseEvent = (PWBMouseCombination.MouseEvents)( EditorGUI.Popup( cellRect,
                                                                                        (int)mouseEvent - 1, _mouseEventsDisplayedOptions )
                                                                                    + 1 );
                                    if ( check.changed )
                                    {
                                        var combi       = new PWBMouseCombination( _modifierOptions[ modId ], mouseEvent );
                    ...
                    if ( _gridCategory )
                    {
                        EditorGUILayout.HelpBox( "These shortcuts work in two steps."
        ...
        private bool BindingConflictDialog( string combi, string conflicts )
            => EditorUtility.DisplayDialog( "Binding Conflict", "The key "
                                                                + combi
        ...
        private void UpdateCombination()
        {
            ...
            if ( Event.current.keyCode    < KeyCode.Space
                 || Event.current.keyCode > KeyCode.F15 ) return;
