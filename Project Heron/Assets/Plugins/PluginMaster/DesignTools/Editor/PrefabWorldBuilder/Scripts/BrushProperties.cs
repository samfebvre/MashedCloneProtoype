...
namespace PluginMaster
{
    public class BrushProperties : EditorWindow, ISerializationCallbackReceiver
    {
        ...
        private GUIStyle _thumbnailToggleStyle;
        private Vector2  _mainScrollPosition = Vector2.zero;

        private       bool            _repaint;
        private       bool            _updateBrushStroke;
        public static BrushProperties instance { get; private set; }

        [MenuItem( "Tools/Plugin Master/Prefab World Builder/Brush Properties...", false, 1120 )]
        public static void ShowWindow() => instance = GetWindow<BrushProperties>( "Brush Properties" );
        ...
        private void OnGUI()
        {
                       ...
                       false, false, GUI.skin.horizontalScrollbar, GUI.skin.verticalScrollbar, GUIStyle.none ) )
            {
                ...
                if ( showBrushGroup )
                {
                    ...
                    #endif
                    if ( _multiBrushGroupOpen )
                    {
                        MultiBrushGroup( ref toggleData );
                    }
                    #if UNITY_2019_1_OR_NEWER
                    EditorGUILayout.EndFoldoutHeaderGroup();
                    #endif
        ...
        private GUIContent _sameStateIcon;
        private GUIContent _mixedStateIcon;
        private GUIContent _changedStateIcon;

        private enum SelectionFieldState
        {
            SAME,
            MIXED,
            CHANGED,
                                                   ...
                                                   BrushSelectionState brushSelectionState, BrushSettings   brushSelectionSettings )
        {
            ...
            {
                brushSelectionSettings = settingsArray[ selection[ 0 ] ].Clone();
            }
            ...
            if ( settingsArray.Length <= selection[ 0 ] )
            {
                return;
            }
            ...
            if ( selection.Length > 0 )
            {
                brushSelectionSettings.Copy( settingsArray[ selection[ 0 ] ] );
            }
        ...
        private GUIContent GetStateGUIContent( SelectionFieldState state )
            => state == SelectionFieldState.SAME
                ? _sameStateIcon
                : state == SelectionFieldState.MIXED
                    ? _mixedStateIcon
                    : _changedStateIcon;
                                           ...
                                           int[]           selection, BrushSelectionState brushSelectionState )
        {
            ...
            {
                BrushSettings brush     = settingsArray[ selection[ i ] ];
                BrushSettings nextBrush = settingsArray[ selection[ i + 1 ] ];
                                           ...
                                           BrushSettings   brushSelectionSettings, BrushSelectionState brushSelectionState )
        {
                ...
                using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                {
                        ...
                        if ( brushSelectionSettings.embedInSurface )
                        {
                            using ( new GUILayout.HorizontalScope() )
                            {
                                ...
                                using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                                {
                                    ...
                                    if ( check.changed )
                                    {
                                        brushSelectionState.embedAtPivotHeight = SelectionFieldState.CHANGED;
                                    }
                                }
                        ...
                        using ( new GUILayout.HorizontalScope() )
                        {
                            ...
                            using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                            {
                                brushSelectionSettings.randomSurfaceDistance
                                    = EditorGUILayout.Popup( "Surface Distance:",
                    ...
                    using ( new GUILayout.HorizontalScope() )
                    {
                        ...
                        using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                        {
                            ...
                            if ( check.changed )
                            {
                                brushSelectionState.localPositionOffset = SelectionFieldState.CHANGED;
                            }
                        }
                ...
                using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                {
                    using ( new GUILayout.HorizontalScope() )
                    {
                        ...
                        using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                        {
                            brushSelectionSettings.alwaysOrientUp
                                = EditorGUILayout.ToggleLeft( "Always orient up",
                                    brushSelectionSettings.alwaysOrientUp );
                            if ( check.changed )
                            {
                                brushSelectionState.alwaysOrientUp = SelectionFieldState.CHANGED;
                            }
                        }
                    ...
                    using ( new GUILayout.HorizontalScope() )
                    {
                        ...
                        using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                        {
                            ...
                            if ( check.changed )
                            {
                                brushSelectionState.rotateToTheSurface = SelectionFieldState.CHANGED;
                            }
                        }
                        ...
                        else
                        {
                            using ( new GUILayout.HorizontalScope() )
                            {
                                ...
                                using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                                {
                                    ...
                                    if ( check.changed )
                                    {
                                        brushSelectionState.eulerOffset = SelectionFieldState.CHANGED;
                                    }
                                }
                ...
                using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                {
                    using ( new GUILayout.HorizontalScope() )
                    {
                        ...
                        using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                        {
                            brushSelectionSettings.randomScaleMultiplier = EditorGUILayout.Popup( "Multiplier:",
                                                                               brushSelectionSettings.randomScaleMultiplier
                                                                                   ? 1
                                                                                   : 0, new[] { "Constant", "Random" } )
                                                                           == 1;
                            if ( check.changed )
                            {
                    ...
                    using ( new GUILayout.HorizontalScope() )
                    {
                        ...
                        using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                        {
                            ...
                            if ( check.changed )
                            {
                                brushSelectionState.separateScaleAxes = SelectionFieldState.CHANGED;
                            }
                        }
            ...
            {
                BrushSettings settings = settingsArray[ idx ];
            ...
            if ( isAsset2D )
            {
                brushFlipGroupOpen = EditorGUILayout.Foldout( brushFlipGroupOpen, "Flip" );
                        ...
                        using ( new GUILayout.HorizontalScope() )
                        {
                            ...
                            using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                            {
                                brushSelectionSettings.flipX
                                    = (BrushSettings.FlipAction)EditorGUILayout.Popup( "Flip X: ",
                                        (int)brushSelectionSettings.flipX, new[] { "No", "Yes", "Random" } );
                                if ( check.changed )
                                {
                                    brushSelectionState.flipX = SelectionFieldState.CHANGED;
                                }
                            }
                        ...
                        using ( new GUILayout.HorizontalScope() )
                        {
                            ...
                            using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                            {
                                brushSelectionSettings.flipY
                                    = (BrushSettings.FlipAction)EditorGUILayout.Popup( "Flip Y: ",
                                        (int)brushSelectionSettings.flipY, new[] { "No", "Yes", "Random" } );
                                if ( check.changed )
                                {
                                    brushSelectionState.flipY = SelectionFieldState.CHANGED;
                                }
                            }
                                          ...
                                          BrushSettings   brushSelectionSettings, BrushSelectionState brushSelectionState )
        {
            using ( new GUILayout.HorizontalScope() )
            {
                ...
                using ( new EditorGUI.DisabledGroupScope( !brushSelectionState.changed ) )
                {
                    ...
                    {
                        UpdateBrushSelectionSettings( selection, settingsArray,
                ...
                if ( GUILayout.Button( "Apply" ) )
                {
                    foreach ( int idx in selection )
                    {
                        ...
                        BrushSettings brush = isItem
                            ? PaletteManager.selectedBrush.GetItemAt( idx )
        ...
        public static bool BrushFields( BrushSettings brush,
                                        ...
                                        Object        objToUndo,         string   undoMsg )
        {
            bool          changed   = false;
            BrushSettings tempBrush = new BrushSettings( brush );
            ...
            using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
            {
                ...
                if ( brushPosGroupOpen )
                {
                    using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                    {
                        using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                        {
                            using ( EditorGUI.ChangeCheckScope checkEmbedSettings = new EditorGUI.ChangeCheckScope() )
                            {
                        ...
                        using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                        {
                            tempBrush.randomSurfaceDistance = EditorGUILayout.Popup( "Surface distance:",
                                                                  tempBrush.randomSurfaceDistance ? 1 : 0, new[] { "Constant", "Random" } )
                                                              == 1;
                            ...
                            {
                                tempBrush.surfaceDistance = EditorGUILayout.FloatField( "Value:",
                ...
                if ( brushRotGroupOpen )
                {
                    using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                    {
                        tempBrush.alwaysOrientUp = EditorGUILayout.ToggleLeft( "Always orient up",
                        ...
                        using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                        {
                            tempBrush.addRandomRotation = EditorGUILayout.Popup( "Add Rotation:",
                                                              tempBrush.addRandomRotation ? 1 : 0, new[] { "Constant", "Random" } )
                                                          == 1;
                            if ( tempBrush.addRandomRotation )
                            {
                                ...
                                using ( new GUILayout.HorizontalScope() )
                                {
                                    ...
                                    tempBrush.rotateInMultiples = EditorGUILayout.ToggleLeft
                                        ( "Only in multiples of:", tempBrush.rotateInMultiples );
                                    using ( new EditorGUI.DisabledGroupScope( !tempBrush.rotateInMultiples ) )
                                    {
                                        tempBrush.rotationFactor
                                            = EditorGUILayout.FloatField( tempBrush.rotationFactor );
                                    }
                                }
                            }
                            else
                            {
                                tempBrush.eulerOffset = EditorGUILayout.Vector3Field( string.Empty,
                    ...
                    using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                    {
                        using ( new GUILayout.HorizontalScope() )
                        {
                            tempBrush.randomScaleMultiplier = EditorGUILayout.Popup( "Multiplier:",
                                                                  tempBrush.randomScaleMultiplier ? 1 : 0, new[] { "Constant", "Random" } )
                                                              == 1;
                            GUILayout.Space( 4 );
                        ...
                        if ( tempBrush.separateScaleAxes )
                        {
                            ...
                            {
                                tempBrush.scaleMultiplier = EditorGUILayout.Vector3Field( string.Empty,
                        ...
                        else
                        {
                            ...
                            {
                                float value = EditorGUILayout.FloatField( tempBrush.scaleMultiplier.x );
                ...
                if ( brush.isAsset2D )
                {
                    ...
                    if ( brushFlipGroupOpen )
                    {
                        using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                        {
                            tempBrush.flipX = (BrushSettings.FlipAction)EditorGUILayout.Popup( "Flip X: ",
                                (int)tempBrush.flipX, new[] { "No", "Yes", "Random" } );
                            tempBrush.flipY = (BrushSettings.FlipAction)EditorGUILayout.Popup( "Flip Y: ",
                                (int)tempBrush.flipY, new[] { "No", "Yes", "Random" } );
                        }
                    }
                }
        ...
        {
            MultibrushSettings brush = PaletteManager.selectedBrush;
            if ( brush == null )
            {
                return;
            }
            ...
            using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
            {
                ...
                {
                    PWBIO.UpdateSelectedPersistentObject();
                }
            }
        }
        ...
        private       bool    _multiBrushGroupOpen;
        private       Vector2 _multiBrushScrollPosition = Vector2.zero;
        private       bool    _multiBrushClipped;

        private bool _itemPosGroupOpen;
        private bool _itemRotGroupOpen;
        private bool _itemScaleGroupOpen;
        private bool _itemFlipGroupOpen;
        private bool _frequencyGroupOpen;
        ...
        private void MultiBrushGroup( ref BrushInputData toggleData )
        {
            ...
            using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
            {
                Rect               brushesRect   = new Rect();
                MultibrushSettings selectedBrush = PaletteManager.selectedBrush;
                ...
                if ( scrollViewRect.Contains( Event.current.mousePosition ) )
                {
                    ...
                    else if ( Event.current.type == EventType.DragPerform )
                    {
                        ...
                        DropUtils.DroppedItem[] droppedItems      = DropUtils.GetDroppedPrefabs();
                        foreach ( DropUtils.DroppedItem droppedItem in droppedItems )
                        {
                            MultibrushItemSettings item = new MultibrushItemSettings( droppedItem.obj, selectedBrush );
                            if ( PWBCore.staticData.undoBrushProperties )
                            {
                                Undo.RegisterCompleteObjectUndo( this, "Add Brush Item" );
                            }
                ...
                using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
                {
                    ...
                    MultibrushItemSettings selectedItem = GetSelectedItem( selectedBrush );
                    if ( selectedItem.prefab == null )
                    {
                        return;
                    }

                    string   itemName      = selectedItem.prefab.name;
                    GUIStyle itemNameStyle = new GUIStyle( EditorStyles.boldLabel );
                    itemNameStyle.alignment = TextAnchor.MiddleCenter;
                    GUILayout.Label( _selectedItemIdx + 1 + ". " + itemName, itemNameStyle );
                    GUIStyle separatorStyle = new GUIStyle( EditorStyles.toolbarButton );
                    ...
                    if ( _selection.Count <= 1 )
                    {
                        using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                        {
                            ...
                            if ( check.changed )
                            {
                                ...
                                if ( selectedItem.overwriteSettings )
                                {
                                    selectedItem.Copy( selectedBrush );
                                }
                            }
                        }
                    }
                    else
                    {
                        using ( new GUILayout.HorizontalScope() )
                        {
                            ...
                            using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                            {
                                ...
                                if ( check.changed )
                                {
                                    _itemSelectionState.overwriteSettings = SelectionFieldState.CHANGED;
                                }
                            }
                    ...
                    if ( ( _selection.Count > 1
                           && ( _itemSelectionState.overwriteSettings == SelectionFieldState.MIXED
                                || ( _itemSelectionState.overwriteSettings != SelectionFieldState.MIXED
                                     && _itemSelectionSettings.overwriteSettings ) ) )
                         || ( _selection.Count <= 1 && selectedItem.overwriteSettings ) )
                    {
                        ItemSettingsGroup();
                    }
                    ...
                    {
                        int[]                    selection     = _selection.ToArray();
                        MultibrushItemSettings[] settingsArray = selectedBrush.items;
                        bool apply = ApplyDiscardButtons( MULTIBRUSH_SETTINGS_UNDO_MSG, true, settingsArray, selection,
                            _itemSelectionSettings,                                     _itemSelectionState );
                        if ( apply )
                        {
                            foreach ( int idx in selection )
                            {
                                MultibrushItemSettings brush = selectedBrush.GetItemAt( idx );
        ...
        private MultibrushItemSettings _newItem;
        ...
        private class ItemSelectionState : BrushSelectionState
        {
            ...
            public override bool changed => base.changed
                                            || embedInSurface == SelectionFieldState.CHANGED
        ...
        {
            int[]                    selection     = _selection.ToArray();
            MultibrushItemSettings[] settingsArray = PaletteManager.selectedBrush.items;

            if ( checkSelectionIndexes )
            {
                for ( int i = 0; i < selection.Length - 1; ++i )
                {
                    int brushIdx     = selection[ i ];
                    int nextBrushIdx = selection[ i + 1 ];
                    if ( brushIdx        >= settingsArray.Length
                         || nextBrushIdx >= settingsArray.Length )
                    {
                        _selection.Clear();
                        _selection.Add( 0 );
                        _selectedItemIdx = 0;
            ...
            for ( int i = 0; i < selection.Length - 1; ++i )
            {
                int                    brushIdx     = selection[ i ];
                int                    nextBrushIdx = selection[ i + 1 ];
                MultibrushItemSettings brush        = settingsArray[ brushIdx ];
                MultibrushItemSettings nextBrush    = settingsArray[ nextBrushIdx ];
                if ( _itemSelectionState.overwriteSettings != SelectionFieldState.CHANGED
        ...
        {
            MultibrushSettings       brush = PaletteManager.selectedBrush;
            MultibrushItemSettings[] items = brush.items;
            for ( int i = 0; i < items.Length; ++i )
            {
                MultibrushItemSettings item = items[ i ];
        ...
        private void SelectPrefabs( object idx )
        {
            ...
            if ( _selection.Contains( (int)idx ) )
            {
                ...
                {
                    GameObject prefab = PaletteManager.selectedBrush.GetItemAt( selectedIdx ).prefab;
                    if ( prefab != null )
                    {
                        prefabs.Add( prefab );
                    }
            ...
            {
                GameObject prefab = PaletteManager.selectedBrush.GetItemAt( (int)idx ).prefab;
                if ( prefab != null )
                {
                    prefabs.Add( prefab );
                }
        ...
        {
            MultibrushItemSettings item = PaletteManager.selectedBrush.GetItemAt( (int)idx );
        ...
        {
            int                    itemIdx = (int)idx;
            MultibrushItemSettings item    = PaletteManager.selectedBrush.GetItemAt( itemIdx );
        ...
        private void CopyThumbnailSettings( object idx )
        {
            ...
            PaletteManager.clipboardOverwriteThumbnailSettings = item.overwriteThumbnailSettings
                ? PaletteManager.Trit.TRUE
                : PaletteManager.Trit.FALSE;
        }
        ...
        private void PasteThumbnailSettings( object idx )
        {
            ...
            if ( _selection.Contains( (int)idx ) )
            {
                ...
                {
                    Paste( PaletteManager.selectedBrush.GetItemAt( i ) );
                }
            }
            else
            {
                Paste( PaletteManager.selectedBrush.GetItemAt( (int)idx ) );
            }
        ...
        private void DeleteItem( object obj )
        {
            ...
            {
                int[] descendingSelection = _selection.ToArray();
                Array.Sort( descendingSelection, ( i1, i2 ) => i2.CompareTo( i1 ) );
                foreach ( int i in descendingSelection )
                {
                    PaletteManager.selectedBrush.RemoveItemAt( i );
                }
            }
            else
            {
                PaletteManager.selectedBrush.RemoveItemAt( idx );
            }
        ...
        private void CreateItemsFromEachPrefabInFolder( object obj )
        {
            ...
            for ( int i = 0; i < items.Length; ++i )
            {
                DropUtils.DroppedItem item = items[ i ];
                if ( item.obj == null )
                {
                    continue;
                }
        ...
        public void CreateItemsFromEachPrefabSelected( object obj )
        {
            ...
            {
                GameObject selectedObj = selectionPrefabs[ i ];
                if ( selectedObj == null )
                {
                    continue;
                }
        ...
        {
            GUIStyle style     = new GUIStyle( _itemStyle );
            int[]    selection = _selection.ToArray();
            if ( PaletteManager.selectedBrush == null )
            {
                return;
            ...
            {
                style.normal = _itemStyle.onNormal;
            }
            ...
            using ( new GUILayout.VerticalScope( style ) )
            {
                ...
                GUILayout.Box( new GUIContent( index + 1 + ". " + item.prefab.name, item.prefab.name ),
            ...
            Rect toggleRect = new Rect( rect.xMax - 16, rect.yMax - 16, 14, 14 );
            using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
            {
                bool include = GUI.Toggle( toggleRect, item.includeInThumbnail, GUIContent.none, _thumbnailToggleStyle );
                if ( check.changed )
                {
                    if ( PWBCore.staticData.undoBrushProperties )
                    {
        ...
        private void DuplicateItem( object obj )
        {
            ...
            {
                PaletteManager.selectedBrush.Duplicate( idx );
            }
        ...
        private void ItemMouseEventHandler( BrushInputData data )
        {
            ...
            if ( data.eventType          == EventType.MouseUp
                 && Event.current.button == 0 )
            {
                ...
                if ( data.shift )
                {
                    int sign = (int)Mathf.Sign( data.index - _selectedItemIdx );
                    if ( sign != 0 )
                    {
                        _selection.Clear();
                        for ( int i = _selectedItemIdx; i != data.index; i += sign )
                        {
                            _selection.Add( i );
                        }
            ...
            {
                DragAndDrop.visualMode = DragAndDropVisualMode.Copy;
                Vector2 size       = new Vector2( 4, data.rect.height );
        ...
        private void ItemSettingsGroup()
        {
            using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
            {
                MultibrushItemSettings item = GetSelectedItem( PaletteManager.selectedBrush );
                if ( _selection.Count <= 1 )
                {
        ...
        private MultibrushItemSettings GetSelectedItem( MultibrushSettings brush )
        {
            ...
            MultibrushItemSettings item = brush.GetItemAt( _selectedItemIdx );
            if ( item == null )
            {
                _selectedItemIdx = 0;
        ...
        private readonly string[] FRECUENCY_MODES = { "Random", "Pattern" };

        private Texture2D      _warningTexture;
        private string         _patternWarningMsg;
        private PatternMachine _previewPatternMachine;

        private void FrequencyGroup()
        {
            using ( new GUILayout.VerticalScope( EditorStyles.helpBox ) )
            {
                using ( EditorGUI.ChangeCheckScope brushCheck = new EditorGUI.ChangeCheckScope() )
                {
                    MultibrushSettings brush                     = PaletteManager.selectedBrush;
                    MultibrushSettings tempBrush                 = brush.Clone() as MultibrushSettings;
                    ...
                    MultibrushItemSettings tempItem = GetSelectedItem( tempBrush );
                        ...
                        using ( new GUILayout.HorizontalScope() )
                        {
                            ...
                            else
                            {
                                ...
                                using ( EditorGUI.ChangeCheckScope check = new EditorGUI.ChangeCheckScope() )
                                {
                                    ...
                                    float totalFrecuency = _itemSelectionSettings.frequency * _selection.Count;
                                    for ( int i = 0; i < PaletteManager.selectedBrush.itemCount; ++i )
                                    {
                    ...
                    else
                    {
                        using ( new GUILayout.HorizontalScope() )
                        {
                            using ( EditorGUI.ChangeCheckScope patternCheck = new EditorGUI.ChangeCheckScope() )
                            {
                                tempBrush.pattern = EditorGUILayout.TextField( "Pattern:", tempBrush.pattern );
                                if ( patternCheck.changed
                                     || tempBrush.patternMachine != _previewPatternMachine
                                     || tempBrush.patternMachine == null )
                                {
                                                ...
                                                out PatternMachine.Token[] tokens ) )
                                    {
                                        ...
                                        default:
                                            ...
                                            {
                                                tempBrush.patternMachine.SetTokens( tokens );
                                            }
                                ...
                                if ( _patternWarningMsg    != null
                                     && _patternWarningMsg != string.Empty )
                                {
                    ...
                    if ( brushCheck.changed
                         && !selectionFrecuencyChanged )
                    {
                        if ( PWBCore.staticData.undoBrushProperties )
                        {
                            Undo.RegisterCompleteObjectUndo( this, MULTIBRUSH_SETTINGS_UNDO_MSG );
                        ...
                        brush.Copy( tempBrush );
                        MultibrushItemSettings selectedItem = GetSelectedItem( PaletteManager.selectedBrush );
